...
While we _could_ adjust our main Project Form to handle file uploads, uploading binary data alters how the server parses standard form fields. A cleaner, safer, and more user-friendly approach is to create a completely independent view strictly for managing a project's media gallery.
...
First, we need a way for administrators to access this dedicated gallery. Open your `views/admin/projects/projects-list.ejs` file and add a "Manage Images" link next to the "Edit" button for each project:
...
Our gallery view sends a multipart payload to `/admin/projects/:projectId/images`. Next, we need to create this dedicated endpoint in `adminRouter.js`, bind our Multer middleware, and set up a `GET` route to render this new page!
