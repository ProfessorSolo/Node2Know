...
**3. Verify Access:**
If we see the prompt, we succeeded. We are now logged in with limited privileges.
...
<NoteBox title="Don't Leave the Engine Running">
  Before moving on, make sure to **exit** the `mongosh` process one last time.
